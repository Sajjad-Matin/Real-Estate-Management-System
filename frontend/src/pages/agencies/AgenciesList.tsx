...
const AgenciesList = () => {
  ...
  const handleExport = (fileFormat: "csv" | "excel") => {
    ...
    const exportData = agencies.map((agency) => ({
      ...
      "Total Users": agency._count?.users || 0,
      "Total Properties": agency._count?.properties || 0,
      "Total Transactions": agency._count?.tradeTransactions || 0,
      "Created Date": format(new Date(agency.createdAt), "MMM d, yyyy"),
    }));

    if (fileFormat === "csv") {
      exportToCSV(exportData, "agencies");
    } else {
      exportToExcel(exportData, "agencies", "Agencies");
    }
  };
  ...
  return (
    <>
      <MainLayout
        title="Agencies"
        subtitle="Manage real estate agencies"
        showExport
